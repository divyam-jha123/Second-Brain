...
export function SecurityBand() {
  ...
  return (
    <section id="security" className="scroll-mt-24 bg-white px-4 py-20 sm:px-6">
      <div className="mx-auto flex max-w-6xl flex-col items-center gap-10 rounded-3xl border border-gray-200 bg-[#1b1b22] p-8 text-white sm:p-12 md:flex-row">
        <div className="flex-1">
          <h2 className="font-hero text-2xl font-bold tracking-tight text-balance sm:text-3xl">
            Your Brain stays yours
          </h2>
          <p className="mt-3 max-w-xl leading-relaxed text-gray-300">
        ...
        <div className="w-full rounded-2xl border border-white/10 bg-white/5 p-5 md:w-[22rem]">
          ...
          <div className="mt-4 flex items-center gap-2.5 rounded-xl border border-white/10 bg-[#131318] px-3 py-2.5">
            <LuLink
              size={14}
              strokeWidth={2}
              className={revoked ? "text-gray-600" : "text-gray-400"}
            />
            <span
              className={`relative truncate font-mono text-[12.5px] tabular-nums transition-colors duration-300 ${
                revoked ? "text-gray-600" : "text-gray-200"
              }`}
            >
              {HASH}
              ...
              <motion.span
                aria-hidden
                className="absolute top-1/2 left-0 h-px w-full origin-left bg-gray-600"
                initial={false}
          ...
          <button
            ...
            className={`mt-4 inline-flex min-h-[44px] w-full items-center justify-center gap-2 rounded-xl px-4 text-sm font-semibold transition-colors duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white ${
              revoked
                ? "border border-white/15 bg-transparent text-gray-300 hover:bg-white/5"
                : "text-white hover:opacity-90"
            }`}
            style={revoked ? undefined : { backgroundColor: ACCENT }}
          >
            {revoked ? (
              <>
