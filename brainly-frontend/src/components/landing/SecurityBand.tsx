...
export function SecurityBand() {
  ...
  return (
    <section id="security" className="scroll-mt-24 bg-white px-4 py-20 sm:px-6">
      <div className="mx-auto flex max-w-6xl flex-col items-center gap-10 rounded-3xl border border-[#1E293B] bg-[#0F172A] p-8 text-white shadow-[0_30px_80px_-48px_rgba(15,23,42,0.65)] sm:p-12 md:flex-row">
        <div className="flex-1">
          <h2 className="font-hero text-2xl font-bold tracking-tight text-balance sm:text-3xl">
            Your Brain stays yours
          </h2>
          <p className="mt-3 max-w-xl leading-relaxed text-[#CBD5E1]">
        ...
        <div className="w-full rounded-2xl border border-white/10 bg-white/5 p-5 md:w-[22rem]">
          ...
          <div className="mt-4 flex items-center gap-2.5 rounded-xl border border-white/10 bg-[#0B1220] px-3 py-2.5">
            <LuLink
              size={14}
              strokeWidth={2}
              className={revoked ? "text-[#475569]" : "text-[#94A3B8]"}
            />
            <span
              className={`relative truncate font-mono text-[12.5px] tabular-nums transition-colors duration-300 ${
                revoked ? "text-[#475569]" : "text-[#E2E8F0]"
              }`}
            >
              {HASH}
              ...
              <motion.span
                aria-hidden
                className="absolute top-1/2 left-0 h-px w-full origin-left bg-[#475569]"
                initial={false}
          ...
          <button
            ...
            className={`mt-4 inline-flex min-h-[44px] w-full items-center justify-center gap-2 rounded-xl px-4 text-sm font-semibold transition-colors duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#60A5FA] ${
              revoked
                ? "border border-white/15 bg-transparent text-[#CBD5E1] hover:bg-white/5 hover:text-white"
